...
function loadLocalSnapshot(): DataSnapshot {
  return {
    ...
    categories: readJson<BlogCategory[]>('categories.json', []),
    posts: readJson<BlogPostRecord[]>('post.json', []),
...
function writeLocalSnapshot(nextSnapshot: DataSnapshot) {
  ...
  writeJson('categories.json', nextSnapshot.categories)
...
async function ensureSchema() {
  ...
  if (!schemaReadyPromise) {
    schemaReadyPromise = (async () => {
      await db.query(`SELECT pg_advisory_lock(${schemaLockId})`)
      try {
        await db.query(`
        ...
        await db.query(`SELECT pg_advisory_unlock(${schemaLockId})`)
      }
    })().catch((error) => {
...
async function loadSnapshotFromDb(): Promise<DataSnapshot> {
  ...
  const [
    ...
    db.query('SELECT data FROM categories ORDER BY data->>\'order\' ASC, id ASC'),
  ...
  return {
    ...
    categories: categoriesResult.rows.map((row) => row.data as BlogCategory),
    visitors: visitorsResult.rows.map((row) => row.data as VisitorRecord),
    posts: postsResult.rows.map((row) => row.data as BlogPostRecord),
    issueComments,
    postComments,
...
async function seedFromLocalIfEmpty() {
  ...
  if ((await countRows('categories')) === 0 && snapshot.categories.length) {
    await upsertRows('categories', snapshot.categories)
  }
...
async function mirrorSnapshotToDb() {
  ...
  try {
    ...
    await Promise.all([
      ...
      replaceRows('categories', snapshot.categories),
...
export async function saveCategories(categories: BlogCategory[]) {
...
export async function savePosts(posts: BlogPostRecord[]) {
