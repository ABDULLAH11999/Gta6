...
import { GlassInset, GlassPanel } from '@/components/ui/glass'
...
export default async function AdminDashboardPage() {
  ...
  const stats = [
    ...
    { label: 'Games', value: settings.games.length, icon: Layers3 },
  ]
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        {stats.map((stat) => (
          <GlassPanel key={stat.label} className="border-white/10 bg-white/[0.04] p-5">
      ...
      <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        <GlassPanel className="space-y-4 border-white/10 bg-white/[0.04]">
        ...
        <GlassPanel className="space-y-4 border-white/10 bg-white/[0.04]">
          ...
          <div className="space-y-3">
            {categories.map((category) => {
              ...
              return (
                <GlassInset key={category.id} className="rounded-3xl">
      ...
      <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
        <GlassPanel className="space-y-4 border-white/10 bg-white/[0.04]">
          ...
          <div className="space-y-3">
            {posts.slice(0, 5).map((post) => (
              <GlassInset key={post.id} className="rounded-3xl">
        ...
        <GlassPanel className="space-y-4 border-white/10 bg-white/[0.04]">
          ...
          <div className="space-y-3">
            ...
            <ActionLink href="/admin/visitors" title="Visitors" text="Review page traffic, sources, and returning sessions." />
...
function SummaryCard({ label, value }: Readonly<{ label: string; value: string }>) {
  return (
    <GlassInset className="rounded-3xl">
...
function ActionLink({
  ...
  return (
    <a href={href} className="block rounded-3xl border border-white/10 bg-black/30 p-4 transition hover:border-white/20 hover:bg-white/5">
